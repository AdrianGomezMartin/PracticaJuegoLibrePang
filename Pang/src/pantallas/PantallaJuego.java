...
/**
 * 
 * @author Adrian Gomez Clase que implementa la interfaz Pantalla para gestionar
 *         el Juego
 *
 */
public class PantallaJuego implements Pantalla {
	...
	public void moverRaton(MouseEvent e) {
		...
		if (personaje.getPosX() > (panelJuego.getWidth() - 200) && nextLevel) {
	...
	private void pintarSiguienteNivel(Graphics g) {
		g.setFont(fuentePuntuacion);
		g.setColor(Color.RED);
		g.drawString("Siguiente Nivel", panelJuego.getWidth() - 200, panelJuego.getHeight() - 20);

	}

	private void cargarSiguienteNivel() {
		PantallaLevel2 level2 = new PantallaLevel2(panelJuego);
		level2.inicializarPantalla();
		level2.setPuntuacion(puntuacion);
		panelJuego.setPantallaActual(level2);
